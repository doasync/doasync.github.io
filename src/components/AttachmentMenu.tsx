...
import { $currentModelSupportsVision, $currentModelSupportsAudio } from '@/features/models-select';
...
export const AttachmentMenu: React.FC<AttachmentMenuProps> = ({ 
  ...
  const [isProcessingFile, modelSupportsVision, modelSupportsAudio] = useUnit([
    $isProcessingFile,
  ...
  const handleStartRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        const audioFile = new File([audioBlob], `recording_${Date.now()}.${mimeType.split('/')[1]}`, {
          type: mimeType,
        });
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      <Tooltip title="Attach files or record audio">
        <span>
          <IconButton
            ...
            sx={{ mx: -0.5 }}
          >
            {isRecording ? <StopIcon /> : <AttachFileIcon />}
          </IconButton>
        </span>
      </Tooltip>
      ...
      >
        <MenuItem onClick={handleImageUpload} disabled={disabled || isProcessingFile}>
          <ListItemIcon>
            <ImageIcon />
          </ListItemIcon>
          <ListItemText 
        ...
        <MenuItem onClick={handleAudioUpload} disabled={disabled || isProcessingFile}>
          <ListItemIcon>
            <AudioFileIcon />
          </ListItemIcon>
          <ListItemText 
        ...
        <MenuItem onClick={handleStartRecording} disabled={disabled || isProcessingFile}>
          <ListItemIcon>
            <MicIcon />
          </ListItemIcon>
          <ListItemText 
