...
export const ChatHistoryView: React.FC = () => {
  const [historyIndex, isLoading, selectChat, removeChat, currentChatId] =
    useUnit([
      $chatHistoryIndex,
      $isLoadingHistory,
      chatSelected,
  ...
  const filteredHistory = useMemo(() => {
    if (!searchTerm) return historyIndex;
  ...
  const handleStartEdit = (id: string, t: string, e: React.MouseEvent) => {
    e.stopPropagation();
    setEditingId(id);
    setEditedTitle(t);
  };
