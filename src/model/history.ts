...
// Events
export const appStarted = createEvent(); // Assuming this might trigger initial load
export const loadChatHistory = createEvent();
export const chatSelected = createEvent<string>(); // Payload is chat ID
export const saveCurrentChat = createEvent(); // Triggered when a chat needs saving/updating
export const deleteChat = createEvent<string>(); // Payload is chat ID
export const newChatCreated = createEvent(); // Event to signal UI reset for new chat
export const chatTitleEdited = createEvent<{ id: string; newTitle: string }>();

// Effects (IndexedDB Operations)
export const loadChatHistoryIndexFx = createEffect<void, ChatHistoryIndex[], Error>(async () => {
...
export const loadSpecificChatFx = createEffect<string, ChatSession | null, Error>(async (id) => {
  const db = await getDb();
  const chat = await db.get(STORE_NAME, id);
  return chat ?? null; // Return null if not found
});

export const saveChatFx = createEffect<ChatSession, void, Error>(async (chatSession) => {
  const db = await getDb();
  await db.put(STORE_NAME, chatSession);
});

export const deleteChatFx = createEffect<string, void, Error>(async (id) => {
  const db = await getDb();
  await db.delete(STORE_NAME, id);
});

// Effect to specifically update a chat's title
export const editChatTitleFx = createEffect<{ id: string; newTitle: string }, ChatHistoryIndex | null, Error>(async ({ id, newTitle }) => {
  const db = await getDb();
  ...
  if (chat) {
    ...
    await tx.done;
    return { id: chat.id, title: chat.title, lastModified: chat.lastModified };
  }
...
const TITLE_PROMPT = 'Summarize the beginning of this chat conversation in 5 words or less. Use title case. Focus on the main topic. Example: Exploring Effector Stores';
...
export const generateTitleFx = createEffect<GenerateTitleParams, GenerateTitleResult, Error>(async ({ chatId, messages, apiKey }) => {
  if (!apiKey) {
  ...
  const apiMessages = [
    ...
    ...messages.slice(0, 4).map(msg => ({ role: msg.role, content: msg.content })),
  ];
  ...
  if (!response.ok) {
    ...
    if (!generatedTitle) {
      console.error('Title generation API response content is empty or undefined:', data);
...
export const $chatHistoryIndex = createStore<ChatHistoryIndex[]>([])
  ...
  .on(deleteChatFx.done, (state, { params: id }) => state.filter(chat => chat.id !== id))
  .on(saveChatFx.done, (state, { params: savedChat }) => {
    ...
    const newEntry = { id: savedChat.id, title: savedChat.title, lastModified: savedChat.lastModified };
    let newState;
    if (existingIndex !== -1) {
      newState = [...state];
   ...
   .on(editChatTitleFx.doneData, (state, updatedIndexEntry) => {
     ...
     return state
       .map(chat => chat.id === updatedIndexEntry.id ? updatedIndexEntry : chat)
       .sort((a, b) => b.lastModified - a.lastModified); // Re-sort after update
   })


export const $currentChatSession = createStore<ChatSession | null>(null, { skipVoid: false })
...
export const $currentChatId = $currentChatSession.map(session => session?.id ?? null);
...
const isChatSession = (chat: ChatSession | null): chat is ChatSession => chat !== null;
...
sample({
  ...
  fn: ({ currentSession, messages, model, temperature, systemPrompt, tokens }, clockTriggerPayload): ChatSession => {
