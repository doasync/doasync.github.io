...
const settingsLoaded = settingsDomain.event<{ apiKey: string; temperature: number; systemPrompt: string }>('settingsLoaded');
// Triggered by UI input changes
export const apiKeyChanged = settingsDomain.event<string>('apiKeyChanged');
export const temperatureChanged = settingsDomain.event<number>('temperatureChanged');
export const systemPromptChanged = settingsDomain.event<string>('systemPromptChanged');

// --- Stores ---
export const $apiKey = settingsDomain.store<string>('', { name: 'apiKey' });
export const $temperature = settingsDomain.store<number>(DEFAULT_TEMPERATURE, { name: 'temperature' });
export const $systemPrompt = settingsDomain.store<string>(DEFAULT_SYSTEM_PROMPT, { name: 'systemPrompt' });
// Store to track if initial settings load is complete
export const $settingsLoaded = settingsDomain.store<boolean>(false, { name: 'settingsLoaded' })
...
const loadSettingsFx = settingsDomain.effect<void, { apiKey: string; temperature: number; systemPrompt: string }, Error>({
    name: 'loadSettingsFx',
    handler: async () => {
        const apiKey = localStorage.getItem(API_KEY_LS_KEY) ?? '';
        const tempRaw = localStorage.getItem(TEMPERATURE_LS_KEY);
        const systemPrompt = localStorage.getItem(SYSTEM_PROMPT_LS_KEY) ?? DEFAULT_SYSTEM_PROMPT;
...
const saveSettingsFx = settingsDomain.effect<{ apiKey: string; temperature: number; systemPrompt: string }, void, Error>({
    name: 'saveSettingsFx',
