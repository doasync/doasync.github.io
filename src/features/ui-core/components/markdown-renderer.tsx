...
// Required for client-side Mermaid rendering in Next.js
...
// Define the code component outside of render to avoid recreating it on every render
function CodeComponent({
  ...
  return !inline && match ? (
    <Highlight code={codeContent} language={match[1]} theme={themes.vsDark}>
      {({
        ...
        >
          {tokens.map((line) => {
            const lineKey = `line-${line.map((t) => t.content).join('')}-${line.length}`;
            return (
              <div key={lineKey} {...getLineProps({ line })}>
                {line.map((token) => {
                  const tokenKey = `${token.content}-${token.types.join('')}-${lineKey}-${token.content.length}`;
