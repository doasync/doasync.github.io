...
- **Streaming Backend:** Utilizes the `chat-stream` feature (`src/features/chat-stream/`) for handling Server-Sent Events (SSE) from the OpenRouter API.
...
- API latency or failures from OpenRouter must be handled gracefully by the `chat-stream` layer and reflected in the mini chat UI (e.g., error message, loading state reset).
