...
import type { DocumentProcessor, DocumentProcessingResult, DocumentProcessingConfig } from "./types";
...
export const processDocumentsFx = documentDomain.effect<File[], DocumentProcessingResult[]>({
  ...
  handler: async (files: File[]) => {
    ...
    if (files.length > 10) {
      throw new Error('Too many files. Maximum 10 files can be processed at once');
    }
    ...
    for (const file of files) {
      try {
        ...
        if (file.size > 100 * 1024 * 1024) { // 100MB absolute limit
          throw new Error(`File is too large. Maximum size: 100MB`);
        }

        const processor = processors.find(p => p.supportedTypes.includes(file.type));
        
        if (!processor) {
          throw new Error(`Unsupported file type: ${file.type}. Supported types: PDF, DOCX, TXT, MD, HTML`);
        }

        if (file.size > processor.maxFileSize) {
          throw new Error(`File is too large for this type. Maximum size: ${Math.round(processor.maxFileSize / 1024 / 1024)}MB`);
        }
        ...
        if (result.extractedText.length > 1000000) { // 1MB text limit
          console.warn(`Large text extracted from ${file.name}: ${result.extractedText.length} characters`);
        }
    ...
    if (errors.length > 0 && results.length > 0) {
      console.warn(`Partial success: ${results.length} files processed, ${errors.length} failed`);
