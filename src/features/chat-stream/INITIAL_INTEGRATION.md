...
- **Imports:**
  - Remove imports related to the old `sendApiRequestFx` and `OpenRouterResponseBody` if no longer needed elsewhere.
...
- **API Call Triggering:**
  ...
  - Modify the `fn` within these `sample` blocks to construct the `StreamChatParams` object required by `streamChatFx`. This includes:
    - Passing the necessary OpenRouter parameters (`model`, `messages`, `temperature`, `apiKey`, etc.).
