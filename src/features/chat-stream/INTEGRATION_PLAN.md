...
Refactor the `chat` and `mini-chat` features to utilize the new `chat-stream` feature for handling OpenRouter API communication. This enables real-time streaming responses and user-triggered cancellation, while avoiding modifications to core message types by having the consumer generate and manage the stream identifier.
...
1.  **Imports:**
    - Remove imports related to the old `sendApiRequestFx`, `sendApiRequestFn`, and `OpenRouterResponseBody` if no longer used.
...
7.  **Cleanup:**
    ...
    - Remove unused types (`OpenRouterResponseBody`, etc.) from `types.ts`.
