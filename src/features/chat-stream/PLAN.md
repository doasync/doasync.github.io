...
- **Objective:** Create a unified, DRY, and robust streaming architecture for OpenRouter API calls using Effector and `eventsource-parser`, located in `src/features/chat-stream/`.
...
## Plan: Unified OpenRouter Streaming Architecture

This plan outlines the design and implementation steps for the `src/features/chat-stream/` feature, providing a unified and maintainable solution for OpenRouter API streaming.

**1. Goal Recap:**

Develop a reusable Effector-based feature (`chat-stream`) that encapsulates OpenRouter SSE streaming logic using `eventsource-parser`. This feature will provide robust streaming, cancellation, and error handling capabilities for consumers like the main chat and MiniChat features.
...
- **`types.ts`**:
  ...
  - Define types for the data payload within SSE events (e.g., `OpenRouterStreamChoice`, `OpenRouterStreamChunk`).
  - Define the parameter type for the main streaming effect (`StreamChatParams`), including OpenRouter API parameters (`model`, `messages`, `temperature`, etc., _ensuring `stream: true` is always set_), API key, and mandatory callback functions (`onChunk`, `onComplete`, `onError`, potentially `onStart`, `onAbort`).
...
- **`api.ts`**:
  ...
  - **Fetch:** Initiate the `fetch` request to OpenRouter with `stream: true` and the provided `AbortSignal`.
