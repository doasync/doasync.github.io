...
import {
  ...
  ttsDialogOpened,
  voiceSelected,
} from '@/features/text-to-speech/model';
import type {
  AudioFormat,
  GeneratedAudio,
} from '@/features/text-to-speech/types';
import { $ttsModels, loadVoiceModels } from '@/features/voice-models';
...
// Helper functions moved outside component to avoid function scoping issues
const createVoiceChangeHandler =
  (onVoiceSelected: (value: string) => void) => (event: SelectChangeEvent) => {
    onVoiceSelected(event.target.value);
  };

const createFormatChangeHandler =
  (onFormatSelected: (value: AudioFormat) => void) =>
  (event: SelectChangeEvent) => {
    onFormatSelected(event.target.value as AudioFormat);
  };

const createModelChangeHandler =
  (onModelSelected: (value: string) => void) => (event: SelectChangeEvent) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <track kind="captions" src="" label="Audio generated from text" />
...
export function TTSDialog({ open, onClose, initialText = '' }: TTSDialogProps) {
  ...
  const handleVoiceChange = createVoiceChangeHandler(voiceSelected);
  const handleFormatChange = createFormatChangeHandler(formatSelected);
  const handleModelChange = createModelChangeHandler(modelSelected);
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, py: 1 }}>
          ...
          {ttsState.selectedModel === 'gpt-4o-mini-tts' && (
            <TextField
              ...
              onChange={(event) => instructionsChanged(event.target.value)}
