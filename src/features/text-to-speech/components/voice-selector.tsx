...
import {
  ...
  previewVoiceFx,
  voiceSelected,
} from '@/features/voice-models';

// Helper function moved outside component to avoid function scoping issues
...
export function VoiceSelector({
  ...
  const handlePreview = (voiceId: string, event: React.MouseEvent) => {
    ...
    if (selectedVoiceModel) {
      previewVoiceFx({ modelId: selectedVoiceModel.id, voiceId }).catch(
        console.error,
      );
    }
  };
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    {showFavorites && (
                      ...
                      >
                        <IconButton
                          size="small"
                          onClick={(event) =>
                            handleToggleFavorite(voice.id, event)
                          }
                        >
                          {isFavorite ? (
                            <FavoriteIcon
                    ...
                    {showPreview && voice.previewUrl && (
                      <Tooltip title="Preview voice">
                        <IconButton
                          size="small"
                          onClick={(event) => handlePreview(voice.id, event)}
                        >
