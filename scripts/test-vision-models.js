...
function getModelProvider(modelId) {
  if (
    ...
    return 'openai';
  if (modelId.includes('claude') || modelId.includes('brainrot'))
    return 'anthropic';
  if (modelId.includes('gemini') || modelId.includes('learnlm'))
    return 'google';
...
class VisionModelTester {
      ...
      if (response.ok) {
        ...
        if (!responseText || responseText === 'No content') {
          if (data.usage?.completion_tokens_details?.reasoning_tokens > 0) {
            responseText =
              '[Model processed the image with reasoning but produced no visible output]';
          }
        }
  ...
  async testAllModels() {
    ...
    for (let i = 0; i < VISION_MODELS.length; i++) {
      ...
      if (result.httpStatus === 429) {
        console.log(
          `Rate limited for ${model}, waiting 20 seconds and retrying...`,
        );
  ...
  generateReport(results) {
    ...
    workingModels.forEach((model) => {
      ...
      if (model.usage) {
        console.log(
          `    Tokens: ${model.usage.total_tokens} (prompt: ${model.usage.prompt_tokens}, completion: ${model.usage.completion_tokens})`,
        );
      }
    ...
    if (workingModels.length > 0) {
      const recommended =
        workingModels.find((m) => m.model.includes('gpt-4o')) ||
        workingModels[0];
    ...
    return {
      ...
      failedModels: failedModels.map((m) => ({
        model: m.model,
        error: m.error,
...
if (typeof process !== 'undefined' && process.env?.VOIDAI_API_KEY) {
  (async () => {
    ...
    const fs = require('fs');
    fs.writeFileSync(
      'vision-models-test-results.json',
      JSON.stringify(
        {
